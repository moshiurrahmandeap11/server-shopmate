  ...
  if (req.files && req.files.images) {
    ...
    for (const image of images) {
      const result = await cloudinary.uploader.upload(image.tempFilePath, {
        folder: "Shopmate_product_images",
        width: 1000,
        crop: "scale",
      });
...
`;
  ...
  // query for fetching top rating products
  const topRatedQuery = `
  SELECT p.*,
  COUNT(r.id) AS review_count
  FROM products p
  ...
  async (req, res, next) => {
    ...
    if (keywords.length === 0) {
      return res.status(200).json({
        success: true,
        message: "No searchable keywords found in your prompt",
        products: [],
      });
    }
    ...
    // Step 2: ai filtering - now properly calling without req, res
    const {success, products: aiProducts, error} = await getAIRecommendation(
      userPrompt, 
      filteredProducts
    );

    if (!success) {
      // If AI filtering fails, return the SQL filtered products as fallback
      return res.status(200).json({
        success: true,
        message: "Products found (AI filtering unavailable, showing SQL matches)",
